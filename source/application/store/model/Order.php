...
class Order extends OrderModel
{
    ...
    {
        // 导出csv文件
        $filename = 'transfer-' . date('YmdHis');
        return export_excel($filename . '.csv', ['订单号', '配送员','运单号']);
    }
    /**
     * 表格值过滤
     * @param $value
    ...
    public function transfer($data)
    {
        ...
        $orderList = [$this];
        // 验证订单是否满足配送条件
        if (!$this->verifyTransfer($orderList)) {
            return false;
        }
        // 整理更新的数据
        $updateList = [[
            'order_id' => $this['order_id'],
            'transfer_people' => $data['transfer_people'],
            'transfer_no' => $data['transfer_no']
        ]];
        // 更新订单配送状态
        if ($status = $this->updateToTransfer($updateList)) {
            // 获取已发货的订单
            $completed = self::detail($this['order_id'], ['user',  'extract_shop','goods']);
            // 发送消息通知
            $this->sendTransferMessage([$completed]);
            // 同步好物圈订单
            //(new WowService($this['wxapp_id']))->update([$completed]);
        }
        return $status;
     ...
     * @throws \Exception
     */
    public function batchTransfer($data)
    {
        // 获取csv文件中的数据
        if (!$csvData = $this->getCsvData()) {
            return false;
     ...
     * @throws \think\exception\DbException
     */
    private function sendTransferMessage($orderList)
    {
        // 实例化消息通知服务类
        $Service = new MessageService;
        foreach ($orderList as $item) {
            // 发送消息通知
            $Service->transfer($item, OrderTypeEnum::MASTER);
        }
        return true;
    }
     ...
     * @throws \Exception
     */
    private function updateToTransfer($orderList)
    {
        $data = [];
        foreach ($orderList as $item) {
            $data[] = [
                ...
                'transfer_people' => $item['transfer_people'],
                'transfer_status' => 20,
                'transfer_time' => time(),
            ];
        }
     ...
     * @return bool
     */
    private function verifyTransfer($orderList)
    {
        foreach ($orderList as $order) {
            if (
                $order['pay_status']['value'] != 20
                ...
                $this->error = "订单号[{$order['order_no']}] 不满足配送条件!";
                return false;
            }
        }
        return true;
     ...
     * @return array|bool
     */
    private function getTransferCsvData()
    {
        // 获取表单上传文件 例如上传了001.jpg
        if (!$file = \request()->file('iFile')) {
            $this->error = '请上传配送模板';
        ...
        $orderList = [];
        while ($item = fgetcsv($csvFile)) {
            if (!isset($item[0]) || empty($item[0]) || !isset($item[1]) || empty($item[1])|| !isset($item[2]) || empty($item[2])) {
                $this->error = '模板文件数据不合法';
                return false;
            }
            $orderList[] = $item;
