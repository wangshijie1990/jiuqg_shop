...
class Message
{
     ...
     * @throws \think\exception\DbException
     */
    public function transfer($order, $orderType = OrderTypeEnum::MASTER)
    {
        // 获取 prepay_id
        $prepayId = $this->getPrepayId($order['order_id'], $orderType);
        // 微信模板消息
        $template = SettingModel::getItem('tplMsg', $order['wxapp_id'])['transfer'];
        if (!$template['is_enable'] || empty($template['template_id'])) {
            return false;
        }
        // 页面链接
        ...
        $status = $this->sendTemplateMessage($order['wxapp_id'], [
            ...
            'data' => [
                ...
                'keyword1' => $order['order_no'],
                // 收货门店
                'keyword2' => $order['extract_shop']['shop_name'],
                // 门店地址
                'keyword3' => ($order['extract_shop']['region']['province'].$order['extract_shop']['region']['city'].$order['extract_shop']['region']['region'].$order['extract_shop']['address']),
                // 收货地址
                'keyword4' => $order['transfer_people'],
                // 物流公司
