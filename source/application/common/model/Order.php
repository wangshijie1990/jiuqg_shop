...
class Order extends BaseModel
{
    ...
    public function getStateTextAttr($value, $data)
    {
        ...
        // 订单类型：单独购买
        if ($data['delivery_status'] == 10 && $data['extract_shop_id']==0) {
            return '已付款，待发货';
        }
        if ($data['transfer_status'] == 10 && $data['extract_shop_id']>0) {
            return '已付款，待配送';
        }
        if ($data['transfer_status'] == 20 && $data['extract_shop_id']>0 && $data['order_status']==10) {
            return '已配送，待提货';
        }
        if ($data['receipt_status'] == 10 && $data['extract_shop_id']==0) {
            return '已发货，待收货';
        }
        if ($data['receipt_status'] == 10 && $data['extract_shop_id']>0) {
            return '已发货，待提货';
    ...
    public function verificationOrder($extractClerkId)
    {
        ...
        return $this->transaction(function () use ($extractClerkId) {
            ...
            ShopOrder::grantMoney($this, $orderType = OrderTypeEnum::MASTER);
            // 获取已完成的订单
