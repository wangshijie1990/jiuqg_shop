...
class Goods extends BaseModel
{
    ...
    protected $append = ['goods_sales','in_limit','quick_status'];
     ...
     * @return false|string
     */
    public function getQuickTimeAttr($value)
    {
        return $value>0?\format_time($value):'';
    }
    /**
     * 获取器：活动开始时间
     * @param $value
     ...
     * @return false|string
     */
    public function getInLimitAttr($value,$data)
    {
    ...
    /*获取商品秒杀状态*/
    public function getQuickStatusAttr($value,$data){
        if($data['quick_open']==10){
            if($data['quick_time']>time()){
                return 'no_begin';
            }
            else{
                return 'has_begin';
    ...
    public function getList($param)
    {
        ...
        $params['in_limit']==10 && ($filter['start_time']=['<=',time()]) && ($filter['end_time']=['>=',time()]);
        $params['tag_value']>0 && $filter['tag_value']=$params['tag_value'];
        $params['quick_open']>0 && $filter['quick_open']=10;
        $params['quick_time']>0 && $filter['quick_time']=$params['quick_time'];
        // 排序规则
        $sort = [];
        if ($params['sortType'] === 'all') {
            ...
            if($hour>6 && $hour<=12){
                ...
                $sort = ['goods_sort', 'goods_id' => 'asc'];
            }else{
                $sort = ['goods_sort', 'goods_name' => 'asc'];
            }
