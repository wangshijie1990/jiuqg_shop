...
class Order extends Controller
{
    ...
    public function agentDetail($order_id)
    {
        // 订单详情
        $order = OrderModel::getAgentOrderDetail($order_id, $this->user['user_id']);
  
        return $this->renderSuccess(compact('order'));
    }
    public function shareDetail($order_id)
    {
        // 订单详情
        $order = OrderModel::getShareOrderDetail($order_id);
        $shop=ShopModel::detail($order['extract_shop_id']);
        $shop->hidden(['freeze_money','money','total_money','phone']);
        $model=new OrderGoodsModel;
        $buylist=$model->getCurrentBuyGoodsList();
        return $this->renderSuccess(compact('order','shop','buylist'));
    }
    /**
     * 获取物流信息
     * @param $order_id
    ...
    public function agentPay($order_id, $payType = PayTypeEnum::WECHAT)
    {
        // 获取订单详情
        $model = OrderModel::getAgentOrderDetail($order_id, $this->user['user_id']);
        // 订单支付事件
        if (!$model->onPay($payType,$this->user['user_id'])) {
            return $this->renderError($model->getError() ?: '订单支付失败');
        }
        // 构建微信支付请求
        $payment = $model->onOrderPayment($this->user, $model, $payType);
