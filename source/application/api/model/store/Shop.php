...
class Shop extends ShopModel
{
     ...
     */
    public function getList($is_check = null, $longitude = '', $latitude = '', $limit = false,$keyword='')
    {
        // 是否支持自提核销
        $is_check && $this->where('is_check', '=', $is_check);
        $keyword && $this->where('shop_name','like','%'.$keyword.'%');
        
        // 获取数量
        $limit != false && $this->limit($limit);
        // 获取门店列表数据
        $data = $this->where('is_delete', '=', '0')->with(['logo'])
            ->where('status', '=', '1')
