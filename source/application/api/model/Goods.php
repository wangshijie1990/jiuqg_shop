...
class Goods extends GoodsModel
{
    ...
    /*获取最近购买这个产品的用户*/
    public function getLastBuy($goodsId){
        $model=new OrderGoods;
        return $model->alias('ordergoods')->join('user','ordergoods.user_id=user.user_id')->join('goods','ordergoods.goods_id=goods.goods_id','right')->Where('goods.is_delete',0)->Where('ordergoods.goods_id',$goodsId)->limit(3)->distinct(true)->field('user.user_id')->field('user.avatarUrl as avatar,user.nickName as username')->select();
    }
    /**
     * 根据商品id集获取商品列表
     * @param $goodsIds
