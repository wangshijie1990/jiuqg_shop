...
use app\api\model\store\shop\Setting as ShopSettingModel;
/**
 * 用户模型类
 * Class User
...
class User extends UserModel
{
    ...
    /*
        ...
        * @throws \think\exception\DbException
    */
    public function getWerun($param){

        $wxConfig = Wxapp::getWxappCache();
        if (empty($wxConfig['app_id']) || empty($wxConfig['app_secret'])) {
            return $this->renderError('请到 [后台-小程序设置] 填写appid 和 appsecret');
        }
        $session=$this->wxlogin($param['code']);
        $model=new WxUser($wxConfig['app_id'], $wxConfig['app_secret']);
        return $model->decryptData($session['session_key'],$param['encryptedData'], $param['iv']);
    }
    /**
     * 微信登录
     * @param $code
    ...
    public function getMenus()
    {
        ...
        //判断门店功能是否开启
        if (ShopSettingModel::isOpen()) {
            $menus['shop']['name'] = ShopSettingModel::getShopTitle();
        } else {
            unset($menus['shop']);
        }
        return $menus;
