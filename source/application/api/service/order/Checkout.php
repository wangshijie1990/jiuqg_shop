...
class Checkout
{
    ...
    private function checkout()
    {
        ...
        // 设置订单商品总金额(不含优惠折扣)
        $this->setOrderTotalPrice($orderData);
        // 当前用户可用的优惠券列表
        $couponList = $this->getUserCouponList($orderData['order_total_price']);

        // 计算优惠券抵扣
        $this->setOrderCouponMoney($orderData, $couponList, $this->param['coupon_id']);
        ...
        if ($this->param['delivery'] == DeliveryTypeEnum::EXPRESS) {
            $this->setOrderExpress($orderData);
        } elseif ($this->param['delivery'] == DeliveryTypeEnum::EXTRACT) {
            $this->param['shop_id'] > 0 && $orderData['extract_shop'] = ShopModel::detail($this->param['shop_id']);
        }
        // 计算订单商品总金额(不含运费)
        $this->setOrderPayPrice($orderData);
        // 返回订单数据
        return array_merge([
    ...
    private function getOrderData()
    {
        ...
        return [
            // 配送类型
            'delivery' => $this->param['delivery'] > 0 ? $this->param['delivery'] : $deliverySetting[0],
            // 默认地址
    ...
    private function validateGoodsList()
    {
        foreach ($this->goodsList as $goods) {
            ...
            // 判断商品是否在秒杀
            if($goods['quick_status']=='no_begin'){
                $this->setError("'很抱歉，商品 [{$goods['goods_name']}] 秒杀还没开始'");
            }
            // 判断商品库存
     ...
     * @param $orderData
     */
    private function setOrderTotalPrice(&$orderData)
    {
        // 订单商品的总金额(不含优惠券折扣)
        $orderData['order_total_price'] = helper::number2(helper::getArrayColumnSum($this->goodsList, 'total_price'));
    }
     ...
     * @param $orderData
     */
    private function setOrderPayPrice(&$orderData)
    {
        // 订单金额(含优惠折扣)
        $orderData['order_price'] = helper::number2(helper::getArrayColumnSum($this->goodsList, 'total_pay_price'));
        // 订单实付款金额(订单金额 + 运费)
     ...
     * @throws BaseException
     */
    private function setOrderCouponMoney(&$orderData, $couponList, $couponId)
    {
        // 设置默认数据：订单信息
        ...
        $orderData['coupon_money'] = helper::number2($CouponMoney->getActualReducedMoney() / 100);
        return true;
    }
     ...
     * @param $orderData
     */
    private function setOrderExpress(&$orderData)
    {
        // 当前用户收货城市id
    ...
    {
        // 表单验证
        if (!$this->validateOrderForm($order, $params['linkman'], $params['phone'])) {
            return false;
        }
        // 创建新的订单
        $status = $this->model->transaction(
            function () use ($order, $params) {
                $status = $this->add($order, $params['remark']);
                if ($status == true) {
                    // 记录收货地址
                    if ($order['delivery'] == DeliveryTypeEnum::EXPRESS) {
                        ...
                        $this->saveOrderExtract($params['linkman'], $params['phone']);
                    }
                    // 保存订单商品信息
                    ...
                    // 记录分销商订单
                    if ($this->checkoutRule['is_dealer']) {
    ...
    private function saveOrderGoods(&$order)
    {
        ...
        foreach ($order['goods_list'] as $goods) {
            ...
            $goodsList[] = [
                ...
                'is_ind_shop' => $goods['is_ind_shop'],
                'shop_money_type' => $goods['shop_money_type'],
                'first_shop_money' => $goods['first_shop_money'],
                'second_shop_money' => $goods['second_shop_money'],
