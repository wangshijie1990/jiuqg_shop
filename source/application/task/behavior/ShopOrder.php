...
use app\task\model\shop\Order as ShopOrderModel;
use think\Log;
/**
 * 门店订单行为管理
 * Class DealerOrder
...
class ShopOrder
{
    ...
    {
        if (!$model instanceof ShopOrderModel) {
            return new ShopOrderModel and false;
        }
    ...
    private function grantMoney()
    {
        ...
        foreach ($list->toArray() as $item) {
            ...
            if ($item['order_master']['order_status']['value'] == 30) {
                $grantIds[] = $item['id'];
                ShopOrderModel::grantMoney($item['order_master'], $item['order_type']['value']);
            }
        }
        // 标记已失效的订单
