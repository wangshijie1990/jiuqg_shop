...
use app\task\model\shop\Clerk as ClerkModel;
...
class Order
{
    ...
    public function run($model)
    {
        ...
        if (!Cache::has("__task_space__order__{$model::$wxapp_id}")) {
            ...
            $this->model->transaction(function () use ($config) {
                ...
                // 已配送的订单自动确认收货
                $this->transfer($config['order']['transfer_days']);
                // 累积用户实际消费金额
                $this->setIncUserExpend($config['order']['refund_days']);
            });
            Cache::set("__task_space__order__{$model::$wxapp_id}", time(), 600);
        }
        return true;
    }
    ...
    private function receive($receiveDays)
    {
        ...
        $filter = [
            ...
            'delivery_status' => 20,
            'receipt_status' => 10,
            'delivery_time' => ['<=', intval($deadlineTime)]
        ];
        // 订单id集
        ...
        $this->dologs('receive', [
            ...
            'deadline_time' => intval($deadlineTime),
            'orderIds' => json_encode($orderIds),
        ]);
        return $status;
    }
    ...
    private function transfer($transferDays)
    {
        if ($transferDays <= 0) {
            return false;
        }
        // 截止时间
        $deadlineTime = time() - ((int)$transferDays * 86400);
        // 条件
        $filter = [
            'pay_status' => 20,
            'transfer_status' => 20,
            'order_status' => 10,
            'transfer_time' => ['<=', intval($deadlineTime)]
        ];
        // 订单id集
        $orderIds = $this->model->where($filter)->column('order_id');
        ...
        */
        ...
        foreach($orderIds as $order_id){
            ...
            $clerk=ClerkModel::detail(array('shop_id'=>$order['extract_shop_id'],'draw'=>10));
            
            if($clerk){
                $status=$order->verificationOrder($clerk['clerk_id']);
            }
        }
        
        // 记录日志
        $this->dologs('transfer', [
            'transfer_days' => (int)$transferDays,
            'deadline_time' => intval($deadlineTime),
            'orderIds' => json_encode($orderIds),
        ]);
        return $status;
    ...
    private function setIncUserExpend($refundDays)
    {
        ...
        $filter = [
            'order_status' => 30,
            'receipt_time' => ['<=', intval($deadlineTime)],     // todo: 改为<=，用于兼容自动确认收货后
            'is_user_expend' => 0
        ];
        // 查询订单列表
